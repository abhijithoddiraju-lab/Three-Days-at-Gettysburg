...
const state = {
    ...
    endlessMode: false,
    terrainSeed: 0,    // when nonzero, spawnTerrain uses seeded RNG (multiplayer sync)
    consoleOpen: false,
};
window.state = state;

// ─── Seeded RNG (Mulberry32) ──────────────────────────────────────────────────
// Used so all peers in a multiplayer session generate identical terrain.
...
window.makeSeededRng = makeSeededRng;
...
function loadUnionTextures(onComplete) {
    const uniqueSources = Array.from(new Set([...UNION_WEAPON_SPRITES, CONFED_SOLDIER_SPRITE, WOUNDED_SOLDIER_SPRITE]));
    ...
    uniqueSources.forEach((path) => {
        ...
        const onLoad = () => {
            if (counted) return;
            counted = true;
            if (++loaded >= total) fire();
        };
        if (tex.baseTexture.valid) { onLoad(); }
        else {
            tex.baseTexture.once('loaded', onLoad);
            tex.baseTexture.once('error', onLoad);
        }
    });
    if (loaded >= total) fire();
}
...
window.addEventListener("mousedown", () => { if (state.gameStarted && !state.paused && state.player && !state.player._downed) state.player.fire(); });
...
window.dist = dist;
window.spawnBloodSplatter = spawnBloodSplatter;
...
window.uiContainer = uiContainer;
    ...
    // Replace your old spawnCommanderPicket in main.js with this:
    function spawnCommanderPicket(x, y) {
        const boss = new Boss(x, y); // Uses the new class from boss.js
        state.enemies.push(boss);

        // Play the epic boss music
        if (musicTracks.overture) {
            playMusic('overture');
        }
    }
...
class World {
    ...
    spawnTerrain() {
        // Use seeded RNG when a multiplayer terrainSeed is set so all peers
        // generate identical terrain; otherwise fall back to Math.random.
        const seed = state.terrainSeed;
        const rng = seed ? makeSeededRng(seed + state.act * 9973) : Math.random;
        ...
        if (act === 1) {
            ...
            // Skip flag pickup in multiplayer (avoids out-of-sync state)
            if (!seed) state.flags.push(new Flag(900, 900, 'union'));
        } else if (act === 2) {
            for (let i = 0; i < 18; i++) {
                const w = 100 + rng() * 120;
                const h = 100 + rng() * 80;
...
class Shell {
    constructor(x, y, angle, type = 'brass') {
        ...
        // Red for shotgun/rifle, Gold for pistol
        const color = type === 'shotgun' ? 0x8B0000 : 0xD4AF37; 
        g.beginFill(color);
        g.drawRect(-2, -1, 4, 2); 
        g.endFill();
        ...
        // Eject at a 90-degree angle from the barrel
        const ejectAngle = angle + (Math.PI / 2) + (Math.random() * 0.4 - 0.2);
        const force = 2 + Math.random() * 2;
        this.vx = Math.cos(ejectAngle) * force;
        this.vy = Math.sin(ejectAngle) * force;
...
class Bullet {
    ...
    update(delta) {
        ...
        const mp = window.mp;
        const mpActive = mp && mp.active;
        const mpPvP = mpActive && (mp.mode === 'pvp_bo5' || mp.mode === 'pvp_dm' || mp.mode === 'pvp_ctf');
        const isLocalBullet = this.owner === state.player;
        // Any RemotePlayer instance carries a _isRemote tag (set in multiplayer.js)
        const isRemoteBullet = mpActive && this.owner && this.owner._isRemote;
        ...
        if (isLocalBullet) {
            ...
            // PvP: visual hit feedback against remote players (each victim is
            // authoritative for its own HP — they'll detect the hit on their
            // own client). We just play a splatter and stop the bullet.
            if (mpPvP && mp.players) {
                for (const rp of mp.players.values()) {
                    if (!rp || rp.dead) continue;
                    if (dist(rp.container.x, rp.container.y, nx, ny) < 22) {
                        spawnBloodSplatter(nx, ny, 55, { baseAngle: Math.atan2(this.vy, this.vx), spread: Math.PI * 0.9 });
                        this.destroy(); return false;
                    }
                }
            }
        } else if (isRemoteBullet) {
            if (mpPvP) {
                ...
                // Co-op: remote player's bullets damage enemies
                for (const e of state.enemies) {
                    if (dist(e.container.x, e.container.y, nx, ny) < 22) {
                        e.takeDamage(this.damage);
                        spawnBloodSplatter(nx, ny, 55, { baseAngle: Math.atan2(this.vy, this.vx), spread: Math.PI * 0.9 });
    ...
    loot() {
        ...
        if (roll < 0.50) {       // Common – ammo
            state.player.weapons.forEach(w => {
                if (w.category === 'rifle') {
                    w.reserve = Math.min(w.reserve + 19, 80);
                }
                if (w.category === 'pistol') {
                    w.reserve = Math.min(w.reserve + 16, 60);
                }
                if (w.category === 'shotgun') {
                    w.reserve = Math.min(w.reserve + 10, 60);
                }
            });
...
class Player {
    constructor(x, y) {
        ...
        this.weapons = [
            ...
            { name: "Blunderbuss", mag: 2, magSize: 2, reserve: 12, color: 0x6a4f34, cooldown: 42, reloadTime: 150, speed: 16, damage: 90, type: 'shotgun', immobileReload: true, pellets: 6, spread: 0.1, category: 'rifle', waveUnlock: 3, active: false, available: false }
            ];
        ...
        this.smokeHealRate  = 48 / 240;     // total heal ~48 HP
    ...
    _buildSprite(idx) {
        ...
        const WEAPON_MAP = {
            'grenade': 'assets/union_granade.png',
            'melee':   'assets/union_sword.png',
            'shotgun': 'assets/union_musket.png'
        };

        let path = null;
        if (weapon) {
            path = WEAPON_MAP[weapon.type] || 
                   (weapon.category === 'pistol' ? 'assets/union_flintlock.png' : null) ||
                   (weapon.category === 'rifle' ? 'assets/union_musket.png' : null) ||
        ...
        if (tex && tex.valid) {
            ...
            // --- SCALE LOGIC ---
            // Default target is 64, but if it's a musket, we boost it to 80 (approx 25% bigger)
            let targetSize = 64;
            if (path && path.includes('union_musket.png')) {
                targetSize = 80; 
            }
            ...
            const offsets = [
                { x:  2, y:  4 }, { x:  0, y:  2 }, { x: -4, y:  2 },
                { x:  0, y:  2 }, { x:  2, y:  2 }, { x:  0, y:  2 },
                { x:  0, y:  2 }, { x:  0, y:  4 }, { x:  0, y:  4 },
            ];
    ...
    fire() {
        ...
        // Helper for casing spawn
        const spawnCasing = (type) => {
            if (!state.shells) state.shells = [];
            const shell = new Shell(this.container.x, this.container.y, this.container.rotation, type);
            this.container.parent.addChild(shell.container);
            state.shells.push(shell);
        };

        if (w.type === 'bullet') {
            const isRifle = w.category === 'rifle';
            // Offset adjusted: 45 for the larger musket, 35 for pistol
            const muzzleOffset = isRifle ? 45 : 35;

            state.bullets.push(new Bullet(
                this.container.x + Math.cos(this.container.rotation) * muzzleOffset,
                this.container.y + Math.sin(this.container.rotation) * muzzleOffset,
                this.container.rotation, w.speed, w.damage, 105, state.player));

            spawnMuzzleFlash(
                this.container.x + Math.cos(this.container.rotation) * (muzzleOffset + 3),
                this.container.y + Math.sin(this.container.rotation) * (muzzleOffset + 3),
                this.container.rotation,
                isRifle ? 1.2 : 0.8);

            state.shake = 1;
            spawnCasing(isRifle ? 'shotgun' : 'brass');

            if (w.category === 'pistol') {
                playSound('pistol', 0.7, 0.12);
            } else {
                playSound('musket', isRifle ? 0.85 : 0.8, 0.1);
            }

        } else if (w.type === 'shotgun') {
            state.shake = 1.2; 
            const muzzleOffset = 42; // Adjusted for larger musket sprite
            ...
            spawnMuzzleFlash(
                this.container.x + Math.cos(this.container.rotation) * (muzzleOffset + 2),
                this.container.y + Math.sin(this.container.rotation) * (muzzleOffset + 2),
            ...
            state.enemies.forEach(e => {
                ...
                // Fixed wrap-around angle logic
                let angleToEnemy = Math.atan2(dy, dx);
                let diff = angleToEnemy - this.container.rotation;
                while (diff < -Math.PI) diff += Math.PI * 2;
                while (diff > Math.PI) diff -= Math.PI * 2;

                if (d < 65 && Math.abs(diff) < 1.1) {
    ...
    takeDamage(dmg) {
        if (this._downed) return;          // already incapacitated
        this.hp -= dmg;
        applyBleed(this, dmg);
    }
    ...
    enterDowned() {
        ...
        const tex = textures[WOUNDED_SOLDIER_SPRITE] || PIXI.Texture.from(WOUNDED_SOLDIER_SPRITE);
        const sprite = new PIXI.Sprite(tex);
    ...
    recoverFromDowned(restoreHp) {
        if (!this._downed) return;
        this._downed = false;
        this.hp = Math.max(1, restoreHp || Math.floor(this.maxHp * 0.5));
        this._buildSprite(this.currentWeaponIndex);
    }
    ...
    update(delta) {
        ...
        // While downed, freeze in place — no movement, aim, abilities or bleed.
        if (this._downed) return;
        ...
        if (this.hp <= 0) {
            app.ticker.speed = 1;
            // In multiplayer, the multiplayer module handles round/match flow.
            // In co-op, instead of dying we enter the "downed" state — a
            // teammate can stand near us and hold V to revive.
            if (window.mp && window.mp.active) {
                this.hp = 0;
                if (window.mp.mode === 'coop' && !this._downed) {
                    this.enterDowned();
                } else if (window.mp.mode !== 'coop') {
...
class HUD {
    ...
    _buildMenu() {
        ...
        const mpSub = new PIXI.Text('Online · PvP · Co-op', { fontFamily:'Georgia,serif', fontSize:12, fill:0xffcccc, fontStyle:'italic' });
        ...
        mpBtn.on('pointerdown', () => {
            if (typeof window.openMultiplayerLobby === 'function') {
                window.openMultiplayerLobby();
                return;
            }

            const existing = document.querySelector('script[src="multiplayer.js"]');
            if (!existing) {
                ...
                script.onload = () => {
                    if (typeof window.openMultiplayerLobby === 'function') {
                        ...
                        alert('multiplayer.js loaded, but the lobby function is unavailable.');
                    }
                };
                script.onerror = () => alert('Unable to load multiplayer.js');
    ...
    update() {
        if (state.gameStarted && this.menu.visible) {
            this.menu.visible = false;
            this.menu.renderable = false;
            this.menu.eventMode = 'none';
        }
...
function getMaxWavesForCurrentAct() {
    // 1. First, check if we should spawn the boss
    if (state.act === 3 && state.wave === 7) {
        // Center of map, top of screen
        spawnCommanderPicket(state.world.width / 2, 100);
    }

    // 2. Then, return the value (this exits the function)
    return state.endlessMode && state.act === 3 
        ? Number.MAX_SAFE_INTEGER 
        : MAX_WAVES_PER_ACT[state.act];
}
...
function spawnWave() {
    ...
    if      (state.objective.type === 'flag')    {
        og.beginFill(0xffff00, 0.18);
        og.drawCircle(0, 0, 24);
        og.endFill();
    }
...
loadUnionTextures(() => {
    ...
    app.ticker.add((delta) => {
        ...
        for (let i = state.particles.length - 1; i >= 0; i--) {
            ...
            // ... end of your particle loop
            } // <--- This is the end of the particle for-loop

            // PASTE THE SHELL UPDATE HERE:
            if (state.shells) {
                for (let i = state.shells.length - 1; i >= 0; i--) {
                    if (state.shells[i].update(delta)) {
                        // Check parent to avoid errors if already removed
                        if (state.shells[i].container.parent) {
                            state.shells[i].container.parent.removeChild(state.shells[i].container);
                        }
...
window.addEventListener("resize", () => {
    ...
    if (state.ui) state.ui.resize();
        ...
        cmdInput.style.bottom = "50px";
        cmdInput.style.left = "50%";
        cmdInput.style.transform = "translateX(-50%)";
        cmdInput.style.width = "400px";
        cmdInput.style.display = "none";
        cmdInput.style.zIndex = "1000";
        cmdInput.placeholder = "Enter command...";
        document.body.appendChild(cmdInput);

        // Listen for the Console Key
        window.addEventListener("keydown", (e) => {
            // 1. ADD THIS: If the console is open, don't let any game keys work
            if (state.consoleOpen) {
                if (e.code === "Escape") closeConsole();
                // Return here so typing doesn't make the player move or shoot
                return; 
            }

            state.keys[e.code] = true;

            // 2. ADD THIS: Open the console when Slash is pressed
            if (e.code === "Slash") {
                e.preventDefault();
                openConsole();
                return;
        ...
        cmdInput.addEventListener("keydown", (e) => {
            if (e.code === "Enter") {
                ...
                const fullText = cmdInput.value.trim().toLowerCase();
                const args = fullText.split(" ");
                const commandName = args.shift(); // The first word (e.g., "wave")

                // 2. Check if the command exists in your Database
                if (CommandDB[commandName]) {
                    // Run the command and get the message back
                    const resultMessage = CommandDB[commandName](args);
                    console.log(`[CMD]: ${resultMessage}`);

                    // Optional: If you want to see the result on screen
                    // alert(resultMessage); 
                } else {
                    console.log(`[CMD]: Unknown command "${commandName}"`);
                }

                // 3. Close the console and clear the box
                closeConsole();
            }
        });

        // Helper function to close the console
        function closeConsole() {
            state.consoleOpen = false;
            cmdInput.style.display = "none";
            cmdInput.value = "";

            // Return focus to the game canvas so you can move again
            app.view.focus();
        }